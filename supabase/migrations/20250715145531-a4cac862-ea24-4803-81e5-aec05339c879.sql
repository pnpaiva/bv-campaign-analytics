...
DROP FUNCTION IF EXISTS get_roster_daily_analytics(UUID[], DATE, DATE);
...
RETURNS TABLE(
  date_recorded DATE,
  total_daily_views NUMERIC,
  total_daily_subscribers NUMERIC,
  total_daily_engagement NUMERIC,
...
BEGIN
  ...
  SELECT 
    ya.date_recorded,
    COALESCE(SUM(ya.daily_views), 0)::NUMERIC as total_daily_views,
    COALESCE(SUM(ya.daily_subscribers), 0)::NUMERIC as total_daily_subscribers, 
    COALESCE(SUM(ya.daily_likes + ya.daily_comments), 0)::NUMERIC as total_daily_engagement,
